...
describe('GameOptionsSettings', () => {
  ...
  it('uses the app bridge fallback when rendered without controlled props', () => {
    ...
    render(<GameOptionsSettings actions={{ 'commit-changes-5': commitChanges }} />);
...
describe('App settings integration', () => {
  ...
  it('resumes from pause with Escape', async () => {
    render(<App />);
    ...
    expect(globalThis.app?.state.currentScreen).toBe('play');
  });
});
